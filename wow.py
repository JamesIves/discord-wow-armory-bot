...
def getData(name, realm, field):
    ...
    path = 'https://us.api.battle.net/wow/character/%s/%s?fields=%s&locale=%s&apikey=%s' % (realm, name, field, REGION, WOW_API_KEY)
...
def characterProgression(name, realm):
    ...
    for raid in info['progression']['raids']:
        ...
        if raid['name'] == 'The Emerald Nightmare':
            emerald_nightmare = calculateBossKills(raid)

        if raid['name'] == 'Trial of Valor':
